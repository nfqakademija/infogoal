...
nginx::resource::vhost { 'infogoal.dev':
  ensure               => present,
  server_name          => [
    'infogoal.dev',
    'www.infogoal.dev'
...
nginx::resource::location { "infogoal.dev-php":
    vhost               => 'infogoal.dev',
    location            => '~ \.php$',
    proxy               => undef,
    www_root            => '/var/www/web/',
...
file { '/usr/local/bin/debug':
  ...
  mode => 755,
  content => "#!/bin/sh\nenv PHP_IDE_CONFIG=\"serverName=infogoal\" XDEBUG_CONFIG=\"idekey=PHPSTORM\" SYMFONY_DEBUG=\"1\" $@"
}
